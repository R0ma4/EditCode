...
namespace EditorCodeIDE.Pack_EditCode.languageRedactors.CSarp
{

    public class ModulCShaerComplit
    {
        public ModulCShaerComplit() { }
    ...
    {
        public int ErrorCoint { get; protected set; }
        public string ErrorMesaege { get; protected set; }
        //
        public string Programm_Name { protected get; set; } = "App.exe";
        public string Programm_Icone { protected get; set; } = "App.ico";
        public int Programm_WarningLevel { protected get; set; } = 2;
        public bool Programm_TreatWarningsAsErrors { protected get; set; } = true;
        public bool Programm_IncludeDebugInformation { protected get; set; } = true;


        protected System.CodeDom.Compiler.CompilerParameters cSharpCodeProvider()
        {
            var Compelator = new System.CodeDom.Compiler.CompilerParameters
            {
                //  ОСНОВНЫЕ 
                GenerateExecutable = false,
                GenerateInMemory = false,
                TreatWarningsAsErrors = Programm_TreatWarningsAsErrors,
                IncludeDebugInformation = Programm_IncludeDebugInformation,
                // ДОПОЛНИТЕЛЬНЫЕ
                WarningLevel = Programm_WarningLevel, // от 0 до 4 {2 - по умочанию}
                CompilerOptions = "/optimize",
                // ПУТИ И ФАЙЛЫ
                OutputAssembly = Programm_Name,
                ...
                // РЕСУРСЫ
                EmbeddedResources = { "resource1.resx" },
                LinkedResources = { "linked.res" },
                Win32Resource = Programm_Icone,
        ...
        public async Task<string> ExecuteCSharpAsync(string code)
        {
            ...
            try
            {
                ...
                CheckForErrorsBackground(exeFile);
        ...
        public void CheckForErrorsBackground(string code)
        {
            try
            {
                using (var provider = new Microsoft.CSharp.CSharpCodeProvider())
                {
                    var parameters = cSharpCodeProvider();
                    ...
                    try
                    {
                        // CompileAssemblyFromSource вместо ( CompileAssemblyFromFile - использовать в будующем, для больших проектов)
                        ...
                        if (results.Errors.HasErrors)
                        {
                            ...
                            {
                                ErrorMesaege = error.ErrorText;
                                ErrorCoint = error.Line;
                            }
                        }
                        else
                        {
                    ...
                    {
                        MessageBox.Show($"Ошибка компиляции: {compileEx.Message}", "Оишбка с кодом LPCP: 003", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Debug.WriteLine($"Ошибка при проверке кода: {compileEx.Message} {compileEx.StackTrace}");
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ошибка компиляции: {ex.Message}", "Оишбка с кодом LPCP: 003", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Debug.WriteLine($"Ошибка при проверке кода: {ex.Message} {ex.StackTrace}");
            }
        }
        /// <summary>
        ...
        public string ErrorComplid(string csFile)
        {
            ...
            try
            {
                using (var provider = new Microsoft.CSharp.CSharpCodeProvider())
                {
                    ...
                    try
                    {
                        // CompileAssemblyFromSource вместо CompileAssemblyFromFile
                        var results = provider.CompileAssemblyFromSource(parameters, csFile);
                    ...
                    {
                        MessageBox.Show($"Ошибка компиляции: {compileEx.Message}", "Оишбка с кодом LPCP: 003.", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Debug.WriteLine($"Ошибка компиляции: {compileEx.Message}");
                        return "Compilert "+compileEx.Message;
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Компилирует C# код в EXE файл используя CodeDom
        /// </summary>
